...
class BGERerankFunction(BaseRerankFunction):
    ...
    def __call__(self, query: str, documents: List[str], top_k: int = 5) -> List[RerankResult]:
        ...
        for index in ranked_order:
            results.append(dict(text=documents[index], score=scores[index], index=index))
        return results
...
class _FlagReranker(FlagReranker):
    def __init__(
        ...
        super().__init__(model_name_or_path=model_name_or_path, use_fp16=use_fp16, cache_dir=cache_dir, devices=device)
